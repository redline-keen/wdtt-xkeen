...
run_installer() {
    ...
    # e2e) — читаем из stdin. На роутере поведение то же самое.
    read_tty() {
...
# 4. Удаление бинарника и конфигураций (включая пул и токен)
...
EOF
    ...
    # Локальный файл рядом со скриптом имеет приоритет (для оффлайн-установки).
    LOCAL_BIN=""
    for cand in "./wdtt-client-$ARCH" "$(dirname "$0")/wdtt-client-$ARCH" "./wdtt-client" "$(dirname "$0")/wdtt-client"; do
        if [ -s "$cand" ]; then LOCAL_BIN="$cand"; break; fi
    done

    if [ -n "$LOCAL_BIN" ]; then
        cp "$LOCAL_BIN" "$INSTALL_DIR/$BIN_NAME"
        echo "Использую локальный бинарник: $LOCAL_BIN"
    else
